...
async function renderBadge(props: { html: string; debounceMs?: number }) {
  const { default: CompatibilityBadge } = await import("./CompatibilityBadge");
...
describe("CompatibilityBadge", () => {
  it("shows '✓ 兼容' when html empty (no API call)", async () => {
    await renderBadge({ html: "", debounceMs: 0 });
  ...
  it("calls /wechat/validate after debounce and shows clean tone", async () => {
    mockReport(0, 0);
    await renderBadge({ html: "<p>hi</p>", debounceMs: 0 });
  ...
  it("shows issue tone when issues > 0", async () => {
    mockReport(2, 0);
    await renderBadge({ html: "<script>x</script>", debounceMs: 0 });
    await waitFor(() => {
      const badge = screen.getByTestId("compat-badge");
  ...
  it("shows warn tone when only warnings", async () => {
    mockReport(0, 3);
    await renderBadge({ html: "<svg/>", debounceMs: 0 });
    await waitFor(() => {
      const badge = screen.getByTestId("compat-badge");
  ...
  it("uses inline SVG icon instead of symbolic glyphs", async () => {
    mockReport(0, 3);
    const { container } = await renderBadge({ html: "<svg/>", debounceMs: 0 });
    await waitFor(() => expect(screen.getByTestId("compat-badge")).toHaveAttribute("data-tone", "warn"));
    const badge = screen.getByTestId("compat-badge");
    expect(badge.textContent ?? "").not.toMatch(/[✓✕⚠]/);
    expect(container.querySelector('[data-testid="compat-badge"] svg')).not.toBeNull();
  });

  it("shows error tone on API failure", async () => {
    postSpy.mockRejectedValue(new Error("net"));
    await renderBadge({ html: "<p>x</p>", debounceMs: 0 });
    await waitFor(() => {
      const badge = screen.getByTestId("compat-badge");
  ...
  it("malformed report (missing issues/warnings arrays) degrades to error tone, never crashes", async () => {
    postSpy.mockResolvedValue({ data: { code: 0, message: "ok", data: {} } });
    await renderBadge({ html: "<p>x</p>", debounceMs: 0 });
    await waitFor(() => {
      const badge = screen.getByTestId("compat-badge");
      expect(badge.getAttribute("data-tone")).toBe("error");
