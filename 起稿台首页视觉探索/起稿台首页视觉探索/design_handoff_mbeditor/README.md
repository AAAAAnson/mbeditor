...
- **禁用 emoji**,所有图标为内联 SVG(见 `ds/icons.jsx`,36 个)。
...
全部 token / 组件的活样张。**先读它**建立全局观,再做具体屏。`ds/icons.jsx`(36 图标)、`ds/motion-spec.jsx`(动效规范，对应 `动效规范.html`)是配套规范页。
...
配套 `editor/editorParts.jsx`、`editor/ProStage.jsx`（pro 三栏舞台）、`editor/DialogLayer.jsx`（唯一对话框层）。
...
### 7.8 对话框/浮层层 — `对话框浮层.html` → `editor/DialogLayer.jsx`(唯一一套)
均挂在 Editor，收敛为**单一** DialogLayer(severity rail + tier 徽标 + 进度 stepper):`SmilWarningDialog`（SVG-SMIL 信息级,复制/草稿都过）· `ValidationWarnToast`（**警告级走轻量 toast 放行**,非拦截式模态——简报 §7-F）· `ValidationBlockDialog`（阻断级,issues 硬 gate,draft+copy 两份,默认不可强过）· `PublishProgress`（复制/草稿进度 stepper,与对话框互斥不叠）· `CopyReadyDialog`（复制二次点击,big=分段）· `CopySuccessPanel`/`DraftSuccessPanel`（可停留确认面板,说人话引导去后台→草稿箱)· `DlMoreMenu`（草稿次级位;`pro` 才露「复制源代码」,已删「导出 HTML」死路)。旧 `editor/Dialogs.jsx`(较弱、`ed-*` 前缀)已删除。
...
- `ds/icons.jsx` — 36 个内联 SVG 图标(禁 emoji)
- `ds/styleguide.jsx`(`设计系统.html`)· `ds/motion-spec.jsx`(`动效规范.html`)— 规范活样张
- `home/HomeSurface.jsx`(`起稿台首页.html`)— 起稿台 `/`
- `compose/IntentInput.jsx` + `AskFlow.jsx`(`写作流意图与提问.html`)· `compose/ConnectAiWizard.jsx`(`连接AI向导.html`)· `compose/GeneratingTheater.jsx`(`流式生成剧场.html`)— 写作流子屏
- `editor/EditorSurface.jsx` + `editorParts.jsx` + `ProStage.jsx` + `DialogLayer.jsx`(`编辑器双预览.html` / `对话框浮层.html`)— 编辑器 + 唯一对话框层
