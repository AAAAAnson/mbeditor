...
(function injectHomeCss() {
  ...
  /* template wall */
  .home-tplgrid{display:grid;grid-template-columns:repeat(auto-fit,minmax(176px,1fr));gap:16px;}
  ...
  /* recent grid */
  .home-grid{display:grid;grid-template-columns:repeat(auto-fit,minmax(280px,1fr));gap:18px;}
              ...
              {articles.map((a) => (
                <div className="home-art" key={a.id}>
                  ...
                  <div className="cover" style={{ background: `linear-gradient(135deg,${a.tone[0]},${a.tone[1]})` }}>
                    <span className="pin" style={{ color: a.tone[2] }}>{React.createElement(coverIcon(a.tone), { size: 22 })}</span>
...
// cover icon derived from the card's tone accent (no hand-hung field)
const TONE_ICON = {
  "var(--orange-400)": IconStroller,
  "var(--orange-500)": IconSparkle,
  "var(--info)": IconBook,
  "var(--success)": IconStore,
  "var(--warning)": IconPin,
};
function coverIcon(tone) { return (tone && TONE_ICON[tone[2]]) || IconPen; }
