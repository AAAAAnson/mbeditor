...
// 依赖 ds/theme.css · ds/icons.jsx · ds/ui.jsx · editor/editorParts · ProStage · DialogLayer(唯一对话层)。
...
function EditorSurface({
  ...
  return (
    <div className={cx("es", narrow && "narrow")}>
      ...
      <div className="es-doc">
        ...
        <div className="es-actions">
          ...
          <div className="es-morewrap">
            ...
            {menu && <DlMoreMenu canPushDraft={canPushDraft} pro={pro} />}
          </div>
        </div>
      </div>
      ...
      {/* ---- dialog / overlay layer (唯一一套 DialogLayer:severity rail + tier + stepper) ---- */}
      {/* 复制路 */}
      {dialog === "copyReady" && <CopyReadyDialog narrow={narrow} />}
      {dialog === "copyBig" && <CopyReadyDialog narrow={narrow} big={true} />}
      {dialog === "copyProgress" && <PublishProgress narrow={narrow} action="copy" at={1} />}
      {dialog === "copySuccess" && <CopySuccessPanel narrow={narrow} canSendToDraft={canPushDraft} />}
      {dialog === "block" && <ValidationBlockDialog narrow={narrow} action="copy" count={2} />}
      {dialog === "smil" && <SmilWarningDialog narrow={narrow} action="copy" />}
      {/* warnings 走轻量 toast 放行(不拦截式模态) */}
      {dialog === "warnToast" && <ValidationWarnToast action="copy" count={2} />}
      {/* 草稿路 —— 复用同一套闸 + SMIL 预警,主按钮文案切「发布」 */}
      {dialog === "draftSmil" && <SmilWarningDialog narrow={narrow} action="draft" />}
      {dialog === "draftBlock" && <ValidationBlockDialog narrow={narrow} action="draft" count={2} />}
      {dialog === "draftWarnToast" && <ValidationWarnToast action="draft" count={2} />}
      {dialog === "draftProgress" && <PublishProgress narrow={narrow} action="draft" at={2} />}
      {dialog === "draftSuccess" && <DraftSuccessPanel narrow={narrow} />}
    </div>
  );
}
