...
// full inline-SVG icon library (kept in sync with ds/icons.jsx exports)
const ICON_SHEET = [
  ["sparkle", IconSparkle], ["template", IconTemplate], ["blank", IconBlank], ["pen", IconPen],
  ["gear", IconGear], ["home", IconHome], ["arrow", IconArrow], ["arrowL", IconArrowL],
  ["chevDown", IconChevDown], ["chevUp", IconChevUp], ["chevLeft", IconChevLeft], ["chevRight", IconChevRight],
  ["clock", IconClock], ["mic", IconMic], ["check", IconCheck], ["close", IconClose],
  ["lock", IconLock], ["copy", IconCopy], ["send", IconSend], ["refresh", IconRefresh],
  ["warn", IconWarn], ["info", IconInfo], ["eye", IconEye], ["eyeOff", IconEyeOff],
  ["book", IconBook], ["store", IconStore], ["pin", IconPin], ["stroller", IconStroller],
  ["external", IconExternal], ["upload", IconUpload], ["image", IconImage], ["plus", IconPlus],
  ["trash", IconTrash], ["moreH", IconMoreH], ["moreV", IconMoreV], ["code", IconCode],
];
  ...
  return (
    <div style={{ maxWidth: 1180, margin: "0 auto", padding: "0 40px 100px" }}>
      ...
      <Section n="04" title="组件基件"
        ...
        {/* icon set — the inline-SVG library (no emoji), one live sheet */}
        <div style={{ marginTop: 20 }}>
          <Panel>
            <Label>图标库 · 内联 SVG(36 个,禁 emoji · stroke 1.8 · round cap/join · 24 视框 · currentColor)</Label>
            <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fill,minmax(108px,1fr))", gap: 10, marginTop: 4 }}>
              {ICON_SHEET.map(([name, C]) => (
                <div key={name} style={{ display: "flex", flexDirection: "column", alignItems: "center", gap: 8,
                  padding: "16px 8px 12px", border: "1px solid var(--line)", borderRadius: "var(--r-md)",
                  background: "var(--surface)" }}>
                  <C size={22} stroke="var(--ink)" />
                  <span style={{ fontFamily: "var(--f-mono)", fontSize: 10.5, color: "var(--ink-soft)" }}>{name}</span>
                </div>
              ))}
            </div>
          </Panel>
