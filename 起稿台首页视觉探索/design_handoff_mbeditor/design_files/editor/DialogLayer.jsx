...
// ════════════════════════════════════════════════════════════════════════
// 2 · ValidationWarnDialog — WARNING tier (可忽略推送). Soft gate: these are
//     人工确认项 the backend only reports. Primary = continue; escape = check.
// ════════════════════════════════════════════════════════════════════════
function ValidationWarnDialog({ narrow, action = "copy", count = 2 }) {
  const verb = action === "draft" ? "发布" : "复制";
  return (
    <DlSheet narrow={narrow} tone="warn" maxWidth={468}
      tier={<><DlTierDot c="var(--warning)" />提醒 · 可忽略</>}
      icon={<IconWarn size={23} stroke="var(--warning-ink)" />}
      title={<>有 {count} 处建议确认,确认后可继续</>}
      sub={<>这些不会被公众号丢弃,但可能和你预想的不太一样。看一眼没问题,就可以直接{verb}。</>}
      footnote={<><IconInfo size={14} stroke="var(--ink-faint)" style={{ flex: "none", marginTop: 1 }} /><span>这些只是提醒,不影响发布——和会被丢弃的「必修项」不同。</span></>}
      footer={
        <div className="dl-foot spread">
          <Button variant="ghost" size="md" leading={<IconEye size={17} />}>返回看看</Button>
          <Button variant="primary" size="md">知道了,继续{verb}</Button>
        </div>
      }>
      <div className="dl-rows" style={{ marginBottom: 2 }}>
        <div className="dl-row warn">
          <span className="ri"><IconWarn size={15} stroke="var(--warning)" /></span>
          <div><div className="rt">有一张图片还没传到素材库</div><div className="rc">img · 第 14 行 · 外链图片</div></div>
        </div>
        <div className="dl-row warn">
          <span className="ri"><IconWarn size={15} stroke="var(--warning)" /></span>
          <div><div className="rt">字号偏小,手机上可能不易读</div><div className="rc">section · 第 22 行 · font-size:12px</div></div>
        </div>
      </div>
    </DlSheet>
  );
...
function DlMoreMenu({ canPushDraft = false }) {
  return (
    <div className="dl-menu" onClick={(e) => e.stopPropagation()}>
      ...
      <button className="dl-mitem"><span className="mi"><IconBlank size={18} /></span><span className="mt">导出 HTML 文件</span></button>
    </div>
  );
}
...
Object.assign(window, {
  ...
  SmilWarningDialog, ValidationWarnDialog, ValidationBlockDialog,
  PublishProgress, CopyReadyDialog, CopySuccessPanel,
});
