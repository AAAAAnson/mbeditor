...
[![Wiki](https://img.shields.io/badge/Wiki-部署_·_网关_·_FAQ-blue.svg)](https://github.com/AAAAAnson/mbeditor/wiki)
...
如果需要一键推送到公众号草稿箱，在编辑器的「设置 → 公众号」页面填入微信公众号的 AppID / AppSecret 并保存。AppSecret 会写入后端具名卷（按 appid，**写后不回显**），刷新页面无需重输;也可直接调 API：
...
凭据存后端 Docker 具名卷 `mbeditor-data:/app/data/credentials.json`（明文 + chmod 600、仓库树外）——永远不会进 git。

> **推草稿报「非白名单 IP」？** 公众号 API 卡的是出口 IP。若你的服务器出口 IP 不便加白名单，可在「设置 → 发布服务器」配置一台固定 IP 的中转网关（或用 `WECHAT_API_BASE` 等环境变量）。详见 [Wiki › 微信网关配置](https://github.com/AAAAAnson/mbeditor/wiki/WeChat-Gateway)。
...
| 方法 | 端点 | 说明 |
|------|------|------|
| `GET` / `PUT` | `/api/v1/settings/credentials` | 公众号 AppSecret(按 appid,写后不回显;GET 只列已配置 appid) |
