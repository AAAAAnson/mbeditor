...
@pytest.mark.parametrize(
    "anim",
    [
        '<animate attributeName="href" to="javascript:alert(1)" begin="click"/>',
        '<set attributeName="href" to="data:text/html,x"/>',
        '<animate attributeName="x" values="0;javascript:alert(1)" dur="1s"/>',
        "<animate attributeName=\"href\" from=\"vbscript:msgbox(1)\" to=\"#a\"/>",
    ],
)
def test_svg_renderer_rejects_script_scheme_in_animation_value(anim):
    """SMIL animate/set must not drive an attribute to a script-execution URL."""
    doc = MBDoc(
        id="d-anim-xss",
        meta=MBDocMeta(title="T"),
        blocks=[
            SvgBlock(
                id="s1",
                source=f'<svg viewBox="0 0 10 10"><rect>{anim}</rect></svg>',
            )
        ],
    )
...
def test_svg_renderer_allows_legit_animation_values():
    """Non-URL animate values (numbers, semicolon lists, opacity) survive."""
    doc = MBDoc(
        id="d-anim-ok",
        meta=MBDocMeta(title="T"),
        blocks=[
            SvgBlock(
                ...
                source=(
                    ...
                    '<animate attributeName="r" values="1;4;1" dur="2s"/>'
                    "</circle></svg>"
                ),
            )
        ],
    )
    out = render_for_wechat(doc, RenderContext(upload_images=False))
    assert "<animate" in out
    assert 'values="1;4;1"' in out
